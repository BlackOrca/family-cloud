...
- `src/FamilyCloud.Storage` — the Storage feature: brokers a bundled OpenCloud instance (self-hosted file collaboration platform, providing folder hierarchy, sharing, WebDAV, and — critically — a Windows desktop client with real Files-on-Demand support) rather than reimplementing file storage. Each family member gets a real OpenCloud account mirroring their FamilyCloud login 1:1 (`OpenCloudAccount`, provisioned/rotated exactly like `CalendarAccount`'s managed Radicale credential), so the official OpenCloud Windows/Android clients work directly with no extra login. `FamilyCloud.Server` only brokers root-folder ("Space") creation and family-scoped sharing authorization (`/api/storage`); `FamilyCloud.App`'s own Storage UI talks to OpenCloud's WebDAV/Graph API directly, not through Server. Referenced only by Server. See "OpenCloud integration" below.
...
- `tests/FamilyCloud.Calendar.Tests`, `tests/FamilyCloud.Family.Tests`, `tests/FamilyCloud.Lists.Tests`, `tests/FamilyCloud.Photos.Tests`, `tests/FamilyCloud.Storage.Tests`, `tests/FamilyCloud.Core.Tests`, `tests/FamilyCloud.Server.Tests` — xUnit tests, mirroring the `src/` feature split.
...
OpenCloud (`opencloud-eu/opencloud`, bundled the same way as Radicale/Immich — an external, unmodified third-party stack orchestrated in `AppHost.cs`) is the Storage feature's backend. Unlike Immich's single shared broker account, each family member gets a real, individual OpenCloud account whose username/password mirror their FamilyCloud login exactly (see `RadicaleCredentialProvisioner` for the precedent this follows) — this is what lets the official OpenCloud Windows desktop client (Files-on-Demand via the Windows Virtual File System) and Android app work directly against the bundled instance with no separate credential to manage. `FamilyCloud.Server` holds one admin/service OpenCloud credential (`OpenCloudServiceAccount`, same shape as `ImmichAccount`) used only to provision per-user accounts and to create/share Spaces on the family's behalf; it never proxies file bytes. `FamilyCloud.App`'s Storage UI calls OpenCloud's WebDAV/Graph API directly with the user's mirrored credentials instead — its `OpenCloudClient` persists that plaintext password in secure storage alongside the JWT (see `AuthTokenStore`/`AppAuthState`), since Basic-Auth against OpenCloud needs it on every request. OpenCloud requires HTTPS even for this internal-only deployment and only has a self-signed certificate, so every `HttpClient` that talks to it (server-side and in the app) overrides certificate validation.
