...
/// SQLite file + data directory per factory instance. Mirrors FamilyCloud.Server.Tests' and
/// FamilyCloud.Family.Tests' copies of the same factory — kept separate per test project rather than
/// shared, since none of these test projects reference each other.
/// </summary>
public sealed class FamilyCloudWebApplicationFactory : WebApplicationFactory<Program>
{
    ...
    private readonly string dataDirectory =
        Path.Combine(Path.GetTempPath(), "familycloud-lists-tests", Guid.NewGuid().ToString("N"));
