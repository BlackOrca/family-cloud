...
public class CalendarWriteServiceTests : IDisposable
{
    ...
    private static (CalendarAccount Account, Calendar.Domain.Calendar Calendar) NewAccountWithCalendar()
    {
    ...
    public async Task DeleteEventAsync_deletes_via_CalDAV_and_removes_the_cached_row()
    {
        ...
        Assert.Equal(new Uri("http://localhost:5232/testuser/household/event-1@familycloud.ics"), deletedUri);
