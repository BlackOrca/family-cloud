...
namespace FamilyCloud.Calendar.Api;
...
public static class CalendarsEndpoints
{
    public static IEndpointRouteBuilder MapCalendarEndpoints(this IEndpointRouteBuilder endpoints)
    {
        ...
        group.MapGet("/", async (HttpContext http, DbContext db) =>
        {
            ...
            var calendars = await db.Set<CalendarPermission>()
        ...
        group.MapGet("/{calendarId:guid}/events", async (
            Guid calendarId, HttpContext http, DbContext db, DateTimeOffset? start, DateTimeOffset? end) =>
        {
            var userId = http.User.GetUserId();
            var hasAccess = await db.Set<CalendarPermission>().AnyAsync(p => p.UserId == userId && p.CalendarId == calendarId);
            if (!hasAccess)
            {
            ...
            var events = (await db.Set<CachedEvent>().Where(e => e.CalendarId == calendarId).ToListAsync())
        ...
        group.MapPost("/{calendarId:guid}/events", async (
            Guid calendarId, EventWriteRequest request, HttpContext http, DbContext db,
            ICalDavPasswordProtector passwordProtector, CalendarWriteService writeService) =>
        {
            var userId = http.User.GetUserId();
            var permission = await db.Set<CalendarPermission>()
        ...
        eventsGroup.MapPut("/{eventId:guid}", async (
            Guid eventId, EventWriteRequest request, HttpContext http, DbContext db,
            ICalDavPasswordProtector passwordProtector, CalendarWriteService writeService) =>
        {
            var userId = http.User.GetUserId();
            var existing = await db.Set<CachedEvent>()
            ...
            var permission = await db.Set<CalendarPermission>().FirstOrDefaultAsync(p => p.UserId == userId && p.CalendarId == existing.CalendarId);
            if (permission is null || !permission.CanWrite)
            {
                return Results.Forbid(authenticationSchemes: [JwtBearerDefaults.AuthenticationScheme]);
            }
        ...
        eventsGroup.MapDelete("/{eventId:guid}", async (
            Guid eventId, HttpContext http, DbContext db,
            ICalDavPasswordProtector passwordProtector, CalendarWriteService writeService) =>
        {
            var userId = http.User.GetUserId();
            var existing = await db.Set<CachedEvent>()
    ...
    private static EventDto ToDto(CachedEvent e) =>
